...
assert.match(mainJs, /const mobileKey=player\.keys\.green\?"GREEN":"---"/, 'Compact mobile HUD must derive visible Green Key state');
assert.match(mainJs, /hudText\(`KEY \$\{mobileKey\}`,135,height-h\+48/, 'Compact mobile HUD must render the key state in the reserved HUD band');
